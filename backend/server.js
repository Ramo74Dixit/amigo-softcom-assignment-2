...
const { bill_0 } = require('./template0'); // Make sure to import the correct template
...
app.post('/generate-bill', (req, res) => {
    ...
    const itemsWithTotal = hsnDetails.map((item, index) => {
        ...
        return {
            hsn_code: item.hsn, // Make sure to use hsn_code
            product_info: item.product_info, // Ensure product_info is included as well
            qty,
            unitPrice,
            totalItemPrice,
            cgstAmount,
